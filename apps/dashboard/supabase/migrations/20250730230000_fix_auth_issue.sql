-- Fix auth issue in pairing function
-- The function should work even if auth.uid() is null (for machine-initiated pairing)

-- Drop and recreate the function to handle auth.uid() properly
...
BEGIN
  ...
  VALUES (p_machine_name, p_location, p_machine_code, current_user_id, true)
